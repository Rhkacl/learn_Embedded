...
#define DEBOUNCE_DELAY  50      // 디바운스 딜레이 50ms
...
uint32_t millis()     // 32bit형 양의 정수 데이터 반환
{
  return HAL_GetTick();
}
...
int main(void)
{
  ...
  uint32_t lastDebounceTime = 0;      // 지역변수라 초기화 해야함
  ...
  while (1)
  {
    ...
    // 버튼 상태
    if(currentButtonState != lastButtonState)   // 버튼 누름
    {
      // 디바운스 시간까지 잠깐 대기
